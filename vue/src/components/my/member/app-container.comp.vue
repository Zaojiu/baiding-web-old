...
<script lang="ts">
  ...
  export default class ActivateComponent extends Vue {
    ...
    async share() {
      if (isInWechat) {
        await initWechat();
        setShareInfo('造就会员', '', `${host.assets}/assets/img/zaojiu-logo.jpg`, `${host.self}/app-member/video`);
      }
    }
    ...
    async init() {
      ...
      switch (this.$route.name) {
        case "app-member.card":
          this.navIndex = 0;
          this.isCard = true;
          break;
        case "app-member.action":
          this.navIndex = 1;
          break;
        case "app-member.plan":
          this.navIndex = 2;
          break;
        case "app-member.video":
          this.navIndex = 3;
          break;
        case "app-member.course":
          this.navIndex = 4;
          break;
        case "app-member.download":
          if (!this.isAndroid) {
            this.navIndex = 5;
          }
          break;
    ...
    changeNav(navIndex: number) {
      switch (navIndex) {
        case 0:
          this.$router.push({path: `/app-member/card`});
          break;
        case 1:
          this.$router.push({path: '/app-member/action'});
          break;
        case 2:
          this.$router.push({path: '/app-member/plan'});
          break;
        case 3:
          this.$router.push({path: '/app-member/video'});
          break;
        case 4:
          this.$router.push({path: '/app-member/course'});
          break;
        case 5:
          if (!this.isAndroid) {
            this.$router.replace({path: '/app-member/download'});
          }
          break;
        default:
      }
    ...
    async payOrder(orderNo: string) {
      let redirectUrl = `${host.self}/app-member/card`;
      if (this.needBack) {
      ...
      if (!isInApp && !isInWechat) {
        // this.$router.push({path: `/app-member/card`, query: {payResult: 'success'}});
      }
      showTips('支付成功');
    }
  }
