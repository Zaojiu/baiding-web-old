<template>
  <form name="postComment" class="post-comment" @submit.prevent="submit" v-focus-first-invalid>
    <header v-if="!isInApp"><i class="bi bi-close" @click="backToTopicPost()"></i>
    ...
    <div class="reply" v-if="replyId">
      <span class="nick">回复 {{replyNick}}:</span>
...
<script lang="ts">
  ...
  export default class PostCommentComponent extends Vue {
    ...
    async backToTopicPost() {
      if (this.isInApp) {
        await initIOS();
        callHandler('popH5State','');
      } else {
        this.$router.push({path: `/topic_post/${this.id}`});
      }
    }
