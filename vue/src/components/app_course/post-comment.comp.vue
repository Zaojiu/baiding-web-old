...
<script lang="ts">
  ...
  import {postComment, postCourseComment} from '../../shared/api/course.api';
  ...
  export default class PostCommentComponent extends Vue {
    ...
    created() {
      ...
      if (request) {
        ...
        this.originalPage = requestObj.originalPage;
      }
    }

    backToCourseItem() {
      if (this.originalPage === 'item') {
        this.$router.push({path: `/app_course/item/${this.id}`});
      } else {
        this.$router.push({path: `/app_course/cover/${this.id}`});
      }
    }
